...
where \(w\) is a smoothing filter, described below.
...
When tested on the image \texttt{dipxetext.tif} the output for $k = 4.5$ gives the result of Figure~\ref{fig:dipxe}.
...
However, if is image is processed using \texttt{IPhighboost} with $k \geq 0.13$ the output image \verb g  will contain a negative value as table~\ref{tbl:outImage} illustrates.
